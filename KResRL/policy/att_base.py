...
class MoE(nn.Module):
    def __init__(self, embed_dim: int, d_ff: int, num_experts: int, k: int = 1):
        ...
        self.experts = nn.ModuleList(
            [
                nn.Sequential(
                    nn.Linear(embed_dim, d_ff), nn.ReLU(), nn.Linear(d_ff, embed_dim)
                )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for i in range(self.k):
            ...
            mask = F.one_hot(
                expert_idx, num_classes=self.num_experts
            ).float()  # (B, L, num_experts)
...
def main():
    ...
    model = SAB(
        embed_dim=128, num_heads=8, d_ff=512, used_moe=False, num_experts=4, moe_top_k=2
    )
