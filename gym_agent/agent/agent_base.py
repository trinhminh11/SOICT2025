...
class AgentBase(ABC):
    ...
    @abstractmethod
    def predict(
        self, observations: NDArray | dict[str, NDArray], deterministic: bool = True
    ) -> ActType:
        """
    ...
    def play(
        self, env: core.EnvWithTransform, stop_if_truncated: bool = True, seed=None, deterministic = True, **kwargs
    ) -> float:
        self.eval = True
        import pygame
        ...
        pygame.init()
        score = 0
        obs = env.reset(seed=seed, **kwargs)[0]
        self.reset()
    ...
    def play_jupyter(
        ...
        **kwargs
        ...
        self.eval = True
        import pygame
        from IPython.display import display # pyright: ignore[reportMissingModuleSource] # type
...
class OffPolicyAgent(AgentBase):
    ...
    def step(
        self,
        observation: NDArray | dict,
        action: NDArray | dict,
        reward: NDArray,
        next_observation: NDArray | dict,
        terminal: NDArray,
        ...
        self.memory.add(observation, action, reward, next_observation, terminal, info)
    ...
    def train_on_episode(self, deterministic: bool, callbacks: Type[Callbacks] = None):
        ...
        while not done:
            ...
            self.step(obs, action, reward, next_obs, terminal, info)
            score += reward.mean()
            obs = next_obs
...
class OnPolicyAgent(AgentBase):
    ...
    @abstractmethod
    def predict(
        self, state: NDArray | dict[str, NDArray], deterministic: bool = True
    ) -> ActType:
        raise NotImplementedError
    ...
    def step(
        ...
        self.memory.add(observation, action, reward, value, log_prob, terminal, info)
    ...
    def train_on_episode(self, deterministic: bool, callbacks: Type[Callbacks] = None):
        ...
        while not done:
            action, value, log_prob, eval_info = self.evaluate(obs, deterministic)
            next_obs, reward, terminal, truncated, info = self.envs.step(action)

            done = (terminal | truncated).all()

            self.step(obs, action, reward, value, log_prob, terminal, info | eval_info)

            score += reward.mean()
            obs = next_obs
            timesteps += 1

        self.memory.calc_advantages_and_returns(value, terminal, eval_info)
